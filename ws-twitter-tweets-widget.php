<?php
/* 
Plugin Name: WS Twitter Tweets Widget   
Plugin URI: http://wordpress.org/plugins/ws-twitter-tweets-widget/
Description: Our WS Twitter Tweets Widget provides easy to add twitter recent tweets on your WordPress site.
Version: 1.0        
Author: Web Shouter    
Author URI: http://www.webshouter.net/  
Tags: twitter,twitter recent tweets, twitter feed, tweet, twitter widget, feed, widget
*/           
                                                                                                                                                  
                                                   
class WS_TWITTER_WIDGET extends WP_Widget {                          
           
    /** constructor */                        
    public function __construct() {          
        parent::__construct(                          
            'ws_twitter_widget', //Base ID
            ' WS Twitter Tweets Widget', //Name
            array( 'description' => __( 'Display tweets from Twitter.', 'webshouter' ), ) //Args
        );
    }
    ...
    function widget( $args, $instance ) { 
        ...
        $instance = wp_parse_args( $instance, array(
            'title'             => 'WS Twitter Tweets Widget  ',
            'query'             => '@twitter',
            'number'            =>  5,
            'show_follow'       => 'false',
            'show_avatar'       => 'true',
    ...
    function form( $instance ) {
        ...
        <p><label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key','webshouter') ?></label><br />
        ...
        <p>
            <label for="<?php echo $this->get_field_id('consumer_secret') ?>"><?php _e('Consumer Secret', 'webshouter') ?></label><br />
        ...
        <p><label for="<?php echo $this->get_field_id('query') ?>"><?php _e('Search Query (<a href="https://dev.twitter.com/docs/using-search" target="_blank" title="Read more about Twitter Search query">?</a>)','webshouter') ?></label><br />
        ...
        <p><label for="<?php echo $this->get_field_id('number') ?>"><?php _e('Number of Tweets','webshouter') ?></label>&nbsp;<input type="text" name="<?php echo $this->get_field_name
        ...
        <p><label for="<?php echo $this->get_field_id('show_follow') ?>">
                ...
                <?php _e('Show Follow Button?', 'webshouter') ?></label>
        </p>
        <p><label for="<?php echo $this->get_field_id('show_account') ?>">
                ...
                <?php _e('Show Account Info?', 'webshouter') ?></label>
        </p>
        <p><label for="<?php echo $this->get_field_id('show_avatar') ?>">
                ...
                <?php _e('Show User Avatar?', 'webshouter') ?></label>
        </p>
...
function ws_register_widget() {
    register_widget( 'WS_TWITTER_WIDGET' );
}
